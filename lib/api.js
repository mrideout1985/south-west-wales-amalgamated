...
export async function getAllCampaignDataWithSlug() {
  const fringles = await client.fetch(`*[_type == "campaigns"]{ 'slug': slug.current }`)
  return fringles;
}
...
export async function getAgreementsData() {
  const data = await client.fetch(`*[_type == 'agreements']{
    policyname, 
    policytype,
    slug,
    url,
